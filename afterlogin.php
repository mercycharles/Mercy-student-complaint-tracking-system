...
<div class="" id="makecomplaint" style="width:100%;display:none;border-radius:15px;padding:15px;margin:10px;background-color:rgba(32, 76, 5, 0.1);border:1px solid aqua;">
    ...
    <form class="form" id="complaint_form" method="POST" style="width:100%;" enctype="multipart/form-data">
        <span id="span2">Nature of Complaint</span><input type="text" name="nature" id="nature" class="form-control in2" placeholder="Regarding to ...."><br />
        <span id="span">Complaint</span>
        <textarea name="comp" class="form-control in1" id="comp" placeholder="Feel free to write. Your complaint is secure." style="height:200px;"></textarea><br />
...
$(document).ready(function() {
    $('#complaint_form').submit(function(event) {
		...
		checkUrgency();
                const formData = new FormData($('#complaint_form')[0]);
                const complaint = $('#comp').val(); // Get the value of the complaint input
...
function checkUrgency() {
    ...
    const complaint = document.getElementById("comp").value;
    formData.append('grievance_text', complaint);
    ...
    .then(data => {
		...
		$.ajax({
                    url: 'usermakecomp.php',
                    type: 'POST',
                    data:{comp:complaint,urgency:urgency,nature:nature},
                    success: function(response) {
						confirm("Successfully submitted. Want to go back?");
							...
							if($result)
								{
									$num=0;
									echo "<table class='table'><tr class='tr'>";
									echo "<th>Number</th><th>Nature</th><th>Date & Time</th>><th>Complaints</th></tr>";
									while($row=mysqli_fetch_assoc($result))
									{ 
										if($user==$row['user'])
										{$num++;
