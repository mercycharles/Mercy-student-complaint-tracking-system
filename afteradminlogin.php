...
if($conn)
{
	...
	$query1 = "SELECT * FROM `complaints` WHERE priority='priority'";
	$result=mysqli_query($conn,$query1);
	$prior=0;
	if($result)
	{
...
<html>
  ...
  <title>Student Complaint Tracking System</title>
  ...
  <script>
			...
			function dashboard()
			{
				...
				var prio=document.getElementById('prior');
    ...
    var inpro = document.getElementById('inprocomp');
    var comp = document.getElementById('complcomp');
    var prior = document.getElementById('prior'); // Change variable name

    if (pend.style.display == 'block') {
        // If 'pending' element is already visible, hide all other elements
        ...
        // If 'pending' element is not visible, show it and hide all other elements
        ...
        comp.style.display = 'none';
				...
				function complcomp()
			{
				...
				var full=document.getElementById('fullhistory');
				var inpro=document.getElementById('inprocomp');
				var pend=document.getElementById('prior');
...
<i class="fa fa-reorder" onclick="callsh()" style="cursor:pointer;font-size:24px;margin:20px 2px 0px 20px;color:white"></i>
		...
		<nav  class="navbar navbar1 bg-dark" style="float:left;margin:0px;display:block;padding:0px;box-shadow:.2px 1px 2px 1px;" id="sh_menu">
			...
			<ul class="navbar-nav" style="width:100%;">
				...
				<li class="nav-item"  id="p_comp" style="display:none;border-top:1.2px solid white;">
					<a class="btn btn-dark btn1" onclick="priority()" href="#">Priority<br />complaints<p id="specialchk"><?php echo $prior;?></p></a>
				</li>
				<li class="nav-item"  id="p_comp" style="display:none;border-top:1.2px solid white;">
		...
		<div id="main" style="width:78%;float:left;" >
			...
			<div id="file1" onclick="priority()" style="cursor:pointer;">
					<i class="fa fa-file-text" style=""></i>
					<p id="file12" style="display:block;font-size:15px;text-align:center;">Priority complaints<div id="pen3"><?php echo $prior; ?></div></p>
				</div>
			...
			<div class="" id="prior" style="width:auto;display:none;border-radius:15px;padding:15px;margin:10px;background-color:rgba(91, 12, 9,.1);border:1px solid aqua;">
			<h4>>Priority Complaints<hr /></h4>
			<form id="searchPriorityForm">
    <div class="form-group">
        <input type="search" class='form-control' style="width:50%;float:left" id="searchPriorityInput" placeholder="Search Priority Complaints">
        <button type="button" class="btn btn-primary" style="margin-top:20px" onclick="searchPriority()">Search</button>
    </div>
</form>
<div id="PriorityResults" style="border:3px solid black"></div>

<script>
   function searchPriority() {
    var searchPriorityValue = $("#searchPriorityInput").val();

    fetch("searchPriority.php", {
        method: "POST",
        headers: {
            "Content-Type": "application/x-www-form-urlencoded"
        },
        body: "searchPriority=" + encodeURIComponent(searchPriorityValue)
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        document.getElementById("PriorityResults").innerHTML = data;
    })
    .catch(error => {
...
document.getElementById("searchPriorityForm").addEventListener("submit", function(event) {
    event.preventDefault();
			...
			<form id="searchPendingForm" action="" method="POST">
    <div class="form-group">
        <input id="searchPendingInput" type="search" class='form-control' style="width:50%;float:left" name="searchPending" placeholder="Search Pending Complaints">
        <button class="btn btn-primary" style="margin-top:20px" name="searchPendingBtn" type="submit">Search</button>
    </div>
</form>
<div id="PendingResults" style="border:3px solid black"></div>

<script>
    document.getElementById("searchPendingForm").addEventListener("submit", function(event) {
        event.preventDefault();
        searchPending();
    });

    function searchPending() {
        var searchPendingValue = document.getElementById("searchPendingInput").value;

        fetch("searchPending.php", {
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
            },
            body: "searchPending=" + encodeURIComponent(searchPendingValue)
        })
        .then(response => {
            if (!response.ok) {
                throw new Error("Network response was not ok");
        ...
        .then(data => {
            document.getElementById("PendingResults").innerHTML = data;
        })
        .catch(error => {
            console.error("Error:", error);
        });
				...
				<?php
					$query1="SELECT * FROM `complaints` where pending='1'";
					$result=mysqli_query($conn,$query1);
						$num=0;
			...
			<form id="searchInProgressForm" action="" method="POST">
    <div class="form-group">
        <input id="searchInProgressInput" type="search" class='form-control' style="width:50%;float:left" name="searchInProgress" placeholder="Search InProgress Complaints">
        <button class="btn btn-primary" style="margin-top:20px" name="searchInProgressBtn" type="submit">Search</button>
    </div>
</form>
<div id="inProgressResults"></div>
	<script>
    document.addEventListener("DOMContentLoaded", function() {
        document.getElementById("searchInProgressForm").addEventListener("submit", function(event) {
            event.preventDefault();
            searchInProgress();
        });
    });
    ...
    function searchInProgress() {
        var searchInProgressValue = document.getElementById("searchInProgressInput").value;
        fetch("searchInProgress.php", {
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
            },
            body: "searchInProgress=" + encodeURIComponent(searchInProgressValue)
        })
        .then(response => {
            if (!response.ok) {
                throw new Error("Network response was not ok");
        ...
        .then(data => {
            document.getElementById("inProgressResults").innerHTML = data;
        })
        .catch(error => {
            console.error("Error:", error);
        });
			...
			<form id="searchCompletedForm" action="" method="POST">
    <div class="form-group">
        <input id="searchCompletedInput" type="search" class='form-control' style="width:50%;float:left" name="searchCompleted" placeholder="Search Completed Complaints">
        <button class="btn btn-primary" style="margin-top:20px" name="searchCompletedBtn" type="submit">Search</button>
    </div>
</form>
<div id="CompletedResults"></div>
	...
	<script>
    document.addEventListener("DOMContentLoaded", function() {
        document.getElementById("searchCompletedForm").addEventListener("submit", function(event) {
            event.preventDefault();
            searchCompleted();
        });
    });
    ...
    function searchCompleted() {
        var searchCompletedValue = document.getElementById("searchCompletedInput").value;
        fetch("searchCompleted.php", {
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
            },
            body: "searchCompleted=" + encodeURIComponent(searchCompletedValue)
        })
        .then(response => {
            if (!response.ok) {
                throw new Error("Network response was not ok");
        ...
        .then(data => {
            document.getElementById("CompletedResults").innerHTML = data;
        })
        .catch(error => {
            console.error("Error:", error);
        });
